...
class Fingerprinter(object):
    ...
    frequency_band_boundary_indices = numpy.zeros(
        FINGERPRINT_NBITS)  # Boundaries of the frequency bands used for fingerprint generation
    ...
    def __init__(self, filepath=None, framewidth=0.37, overlap=31.0 / 32.0):
        ...
        self.fingerprints = numpy.zeros(int(self.nframes))
        self.fingerprints_binary = numpy.zeros((int(self.nframes), self.FINGERPRINT_NBITS), dtype=numpy.bool)
    ...
    def init_fingerprints(self):
        ...
        last_frame = data[0:int(self.nsamples_per_frame)]
        ...
        while there_are_frames_left:
            current_frame = data[int(position):int(position) + int(self.nsamples_per_frame)]
            ...
            self.fingerprints_binary[index_fingerprint] = self.generate_binary_fingerprint_from_frames(fft_last_frame,
                                                                                                       fft_current_frame)
    ...
    def calculate_frequency_bands(self, nsamples_per_frame, frame_length):
        ...
        index_lower_limit = int(
            math.floor(frame_length * self.FREQUENCY_BAND_LOWER_LIMIT))  # index of 300Hz freq component
        index_upper_limit = int(
            math.ceil(frame_length * self.FREQUENCY_BAND_UPPER_LIMIT))  # index of 2000Hz freq component
    ...
    def calculate_band_energies_from_frame(self, frame):
        ...
        for band in range(self.FINGERPRINT_NBITS + 1):
            energies[band] = numpy.sum(numpy.abs(frame[int(self.frequency_band_boundary_indices[band]): int(
                self.frequency_band_boundary_indices[band + 1])]) ** 2)
        return energies
    ...
    def time_to_frameindex(self, seconds):
        # transform the seconds into frame number
        index = int(seconds * self.sample_rate / self.nsamples_between_frames)
        return index

    # Find the best match between this fingerprinter and a given fingerprinter
    def find_position(self, fingerprinted, start_frame = None, end_frame = None):
        # start_frame = None
        # end_frame = None
